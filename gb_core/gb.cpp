...
bool gb::load_rom(byte *buf, int size, byte *ram, int ram_size, bool persistent, const char* rom_name)
{
    if (m_rom->load_rom(buf, size, ram, ram_size, persistent, rom_name))
    {
        reset();
        return true;
    }
